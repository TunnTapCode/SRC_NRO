    ...
    public void checkDoneTaskGoToMap(Player player, Zone zoneJoin) {
        if (player.isPl()) {
            switch (zoneJoin.map.mapId) {
                ...
                case 99 -> doneTask(player, ConstTask.TASK_26_0);
    ...
    public void checkDoneTaskKillBoss(Player player, Boss boss) {
        if (player != null && !player.isBoss && !player.isPet) {
            ...
            switch ((int) boss.id) {
                ...
                case BossID.SIEU_BO_HUNG -> doneTask(player, ConstTask.TASK_27_4);
                case BossID.DRABURA -> doneTask(player, ConstTask.TASK_28_1);
                case BossID.BUI_BUI -> doneTask(player, ConstTask.TASK_28_2);
                case BossID.BUI_BUI_2 -> doneTask(player, ConstTask.TASK_28_3);
                case BossID.YA_CON -> doneTask(player, ConstTask.TASK_28_4);
                case BossID.DRABURA_2 -> doneTask(player, ConstTask.TASK_28_5);
                case BossID.MABU -> doneTask(player, ConstTask.TASK_28_6);
    ...
    private void rewardDoneTask(Player player) {
        ...
        if (player.playerTask.taskMain.id > 0 && player.playerTask.taskMain.id < 25) {
            ...
            if (player.playerTask.taskMain.id > 0 && player.playerTask.taskMain.id < 25) {
                ...
                // Chỉ nhận 1 vàng khi hoàn thành nhiệm vụ
        ...
        if (player.playerTask.sideTask.template != null) {
            if (player.playerTask.sideTask.isDone()) {
                ...
                if (InventoryService.gI().getCountEmptyBag(player) > 0) {
                    ...
                    Item item = ItemService.gI().createNewItem((short) itemId, itemQuantity);
                    ...
                    Service.gI().sendThongBao(player,
                            "Bạn nhận được x" + itemQuantity + " " + item.template.name
                                    + " và " + Util.numberToMoney(goldReward) + " vàng");
